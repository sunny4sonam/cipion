...
<head>
    <title><fmt:message key="gradeDetail.title"/></title>
    <meta name="heading" content="<fmt:message key='gradeDetail.heading'/>"/>
</head>

<form:form commandName="grade" method="post" action="gradeform.html" id="gradeForm" onsubmit="return validateGrade(this)">
    <div class="subnavmenu-wrapper">
		<div class="subnavmenu-left">
		</div>
		<div class="subnavmenu">
			<!-- si no hay sid es uno nuevo, mostramos done and back -->
			<c:if test="${empty grade.sid}">
    			<a id="doneButton" href="#" onclick="this.blur();document.forms['gradeForm'].submit();" title="<fmt:message key="button.save"/>">
    			</a>
			...
			<c:if test="${not empty grade.sid}">
    			...
    			<a id="deleteButton" name="delete" onclick="bCancel=true;if (confirmDelete('<fmt:message key="grade.delete"/>')){document.forms['gradeForm'].action='gradeform.html?delete=true';document.forms['gradeForm'].submit();} " href="#" title="<fmt:message key="button.delete"/>">
    			</a>
    			<div class="buttonSeparator">
				</div>
			</c:if>
			<a id="backButton" href="grades.html" onclick="this.blur();" title="<fmt:message key="button.back"/>">
			</a>
	...
	<div id="stylized" class="myform" >
		...
		<c:if test="${param.edit=='false'}">
			<a id="editButton" href="gradeform.html?sid=${grade.sid}&edit=true" title="<fmt:message key="button.edit"/>">
        ...
        <form:hidden path="sid"/>
            <li>
                ...
                <label><fmt:message key="grade.name"/>
                    <span class="small"><fmt:message key="grade.name.detail"/></span>
                </label>				
				<form:input path="name" id="name" cssErrorClass="text medium error" maxlength="150" readonly="${param.edit=='false'}"/>
            </li>
            <li>
                <form:errors path="description" cssClass="fieldError"/>
                <label><fmt:message key="grade.description"/>
                    <span class="small"><fmt:message key="grade.description.detail"/></span>
                </label>				
